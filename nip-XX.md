...
This NIP defines a set of tags to set zap parameters for note payments. These parameters allow users to add optional payment conditions and instructions for their notes such as a minimum and maximum zap amount, redirecting the zap to a different user, or specific LNURL to receive zaps for that note. This can enable supporting clients to better emulate payment behavior in notes via zaps.
 
## Zaps as Public Payments

 This NIP aims to create a standard that extend zaps interaction for users that want to associate predefined payment conditions to notes, such as raising funds or settling bills, while creating a public proof that the payer and payee have agreed on the transaction.
...
These tags are to be appended to the note tags list. 

###  `zap-min` - Minimum amount requested for public payment.

`["zap-min", zapMin]`
`zapMin: Minimum target amount in milisats.`
  
### `zap-max` - Maximum amount requested for public payment.

`["zap-max", zapMax]`
`zapMax: Minimum target amount in milisats.`
  
###  `zap-uses` - Number of public payments requested.

`["zap-uses", zapUses]`
`zapUses: Number of expected zaps with an amount between (and including) zap-min and zap-max.`

###  `zap-lnurl` - Request to override the destination of public payment.

`["zap-lnurl", zapLNURL]`
`zapLNURL: Custom LN Address or LNURL override.`

This zap tag allows requesters to override the default behavior of zapping the Lightning (LN) address linked to the creator's kind 0 post. It opens up possibilities for businesses that require each payment to be tied to a specific LNURL or need to redirect payments to another user.

If the payer's client does not support this behavior, or if the LNURL is unresponsive, the system will fall back to the default behavior. Optionally, the zap event can include this tag to differentiate such cases for better traceability.
